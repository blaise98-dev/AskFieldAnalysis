...
class PivotTableUI extends React.PureComponent {
  ...
  render() {
    ...
    const rendererCell = (
      ...
      <Select
        options={Object.keys(this.props.renderers).map((value) => ({
          value,
          label: value,
        }))}
        value={{
        ...
        isMenuOpen={this.isOpen('renderer')}
        ...
        onChange={(selectedOption) => {
          this.propUpdater('rendererName')(selectedOption.value);
        }}
      />
    </td>
...
<td className="pvtVals">
  <Select
    options={Object.keys(this.props.aggregators).map((value) => ({
      value,
      label: value,
    }))}
    value={{
      value: this.props.aggregatorName,
      label: this.props.aggregatorName,
    }}
    isMenuOpen={this.isOpen('aggregators')}
    menuPlacement="bottom"
    ...
    onChange={(selectedOption) => {
      this.propUpdater('aggregatorName')(selectedOption.value);
    }}
  />

  {/* Row and Column order buttons */}
  <a
    role="button"
  ...
  {new Array(numValsAllowed).fill().map((n, i) => (
    <div key={i}>
      <Select
        options={Object.keys(this.state.attrValues).filter((e) =>
        ...
        value={{
          ...
          label: this.props.vals[i],
        }}
        isMenuOpen={this.isOpen(`val${i}`)}
        menuPlacement="bottom"
        ...
        onChange={(selectedOption) => {
          const updatedVals = [...this.props.vals];
          updatedVals[i] = selectedOption.value;
          this.sendPropUpdate({ vals: updatedVals });
        }}
      />
      {i + 1 !== numValsAllowed && <br />}
    </div>
  ))}

  {/* Render aggregatorCellOutlet */}
...
<tr>
  {aggregatorCell}
</tr> <tr className='row-designer'>
{unusedAttrsCell} row designer
</tr> 
<tr className='row-dropped'>
{colAttrsCell}</tr>
</span> 
{colAttrsCell}
<span>
  <tr className='column-designer'>
  {unusedAttrsCell} column designer
  </tr>
  <tr className='column-dropped'>
    {rowAttrsCell }
  </tr>
  <tr>
    {outputCell}
